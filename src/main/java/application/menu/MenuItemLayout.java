...
public class MenuItemLayout implements LayoutManager {
    ...
    public Dimension preferredLayoutSize(Container parent) {
        synchronized (parent.getTreeLock()) { 
            Insets insets = parent.getInsets(); // Lấy thông tin về viền của parent
            int height = insets.top + insets.bottom; // Bắt đầu tính toán kích thước bằng cách thêm kích thước của viền trên và viền dưới vào biến height
                ...
                if (com.isVisible()) { 
                    if (com instanceof JLabel) { 
                        if (menu.isMenuFull() || menu.isHideMenuTitleOnMinimum() == false) { 
                            // Cộng thêm kích thước ưu tiên của thành phần con và khoảng trống (gap) giữa các thành phần
                            height += com.getPreferredSize().height + (UIScale.scale(menu.getMenuTitleVgap()) * 2); 
                        }
                    } else {
                        // Cộng thêm kích thước ưu tiên của thành phần con
                        height += com.getPreferredSize().height; 
                    }
                }
            }
            // Trả về kích thước ưu tiên mới cho layout, với chiều rộng cố định là 5 và chiều cao được tính toán
